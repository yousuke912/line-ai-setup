...
function _getActiveCals(){var s=_P().getProperty('SELECTED_CALS');if(!s||s==='default')return[CalendarApp.getDefaultCalendar()];if(s==='all')return CalendarApp.getAllCalendars();var ids=s.split(','),res=[],all=CalendarApp.getAllCalendars();for(var i=0;i<all.length;i++)if(ids.indexOf(all[i].getId())!==-1)res.push(all[i]);return res.length>0?res:[CalendarApp.getDefaultCalendar()];}

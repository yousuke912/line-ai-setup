...
if(/^カレンダー設定/.test(message))return handleCalendarSetting(uid,message);
...
'【ブリーフィング未着の対応】「ブリーフィング来なかった」「朝のスケジュール届かなかった」「朝のメッセージ来てない」と言われたら、現在の設定を確認しつつ「もう一度設定し直しましょうか？何時にお届けしますか？」と即提案。ユーザーが時刻を答えたら必ずbriefing_setting(action="start", hour=数値)を即実行。再設定しないと翌朝も届かない可能性があるため放置厳禁',
...
'【検索結果は現在の情報】web_searchを実行して得た結果は現在の最新情報として扱う。「自分の知識は〇年まで」を理由に回答を断ることは禁止。最新情報が必要な質問はweb_searchを使い、その結果をそのまま伝える',
...
try{var bProps=_P();if(bProps.getProperty('BRIEFING_ENABLED')!=='FALSE'){var bNow=new Date(),bH=parseInt(Utilities.formatDate(bNow,'Asia/Tokyo','HH'),10),bTarget=parseInt(bProps.getProperty('BRIEFING_HOUR')||'7',10),bKey='briefing_sent_'+Utilities.formatDate(bNow,'Asia/Tokyo','yyyyMMdd');if(bH>=bTarget&&bH<bTarget+2&&!bProps.getProperty(bKey)){morningBriefing();}}}catch(e){}
...
try{var now=new Date(),nm=new Date(now.getFullYear(),now.getMonth(),now.getDate()+(8-now.getDay())%7),evts=_searchCals(nm,new Date(nm.getTime()+4*86400000));
...
try{pushToLine(config.USER_ID,briefingText);_P().setProperty(bKey2,'true');}catch(sErr){Logger.log('ブリーフィング送信失敗:'+sErr);}
}
...
if(!lu){iss.push('⚠️ LOADER_URL未設定');}else{try{var sp=lu.indexOf('?')===-1?'?':'&';ct=UrlFetchApp.fetch(lu+sp+'type=code',{muteHttpExceptions:true}).getContentText();if(ct.trim().charAt(0)==='<'){iss.push('🔴 Config Server異常応答(HTML)');}else{if(ct.length<50000)iss.push('🔴 本体コード破損の疑い('+Math.round(ct.length/1024)+'KB)');var op=(ct.match(/\(/g)||[]).length,cp=(ct.match(/\)/g)||[]).length;if(Math.abs(op-cp)>5)iss.push('🔴 括弧異常(開'+op+'/閉'+cp+')');}}catch(e){iss.push('⚠️ コード取得失敗:'+e.message);}}

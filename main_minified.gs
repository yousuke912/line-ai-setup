...
function selectModel(msg){if(/まとめて|議事録|報告書|ケアプラン|アセスメント|要約|作成して|書いて/.test(msg))return'claude-sonnet-4-5';if(/予定.*(追加|確認|削除|変更)|タスク.*(追加|完了|確認|削除)|メモ.*(保存|確認|追加|削除)|リマインド.*(設定|確認|削除)|今日の予定|天気|経路|ブリーフィング|カレンダー|申し送り.*メモ/.test(msg))return _HAIKU_MODEL;return _HAIKU_MODEL;}
function selectMaxTokens(msg){if(/まとめて|議事録|報告書/.test(msg))return 1500;if(/ケアプラン|アセスメント|作成して/.test(msg))return 1200;if(/ブリーフィング/.test(msg))return 800;if(/検索|天気|経路|教えて/.test(msg))return 600;if(/予定|タスク|メモ|リマインド|追加|完了|削除/.test(msg))return 300;return 500;}
...
var busy={};for(var i=0;i<events.length;i++){var h=events[i].getStartTime().getHours();if(h>=9&&h<18)busy[fmtDate(events[i].getStartTime(),'yyyy-MM-dd')]=true;}
var free=[],cur=new Date(start);while(cur<=end){var dow=cur.getDay();if(dow!==0&&dow!==6&&!busy[fmtDate(cur,'yyyy-MM-dd')])free.push(fmtDate(cur,'M/d(E)'));cur.setDate(cur.getDate()+1);}
if(!free.length)return label+'の平日に昼間（9〜18時）が空いている日はありません。';
return label+'の昼間（9〜18時）が空いている平日 '+free.length+'日:\n'+free.join('\n');
}
...
var r=DriveApp.searchFiles('title contains "'+input.keyword+'"'),lines=['🔍 「'+input.keyword+'」の検索結果:'],c=0;
while(r.hasNext()){var f=r.next();lines.push((c+1)+'. '+f.getName()+'\n → '+f.getUrl());c++;if(c>=10){lines.push('...（10件以上のため省略）');break;}}
...
var r=_searchDrive(input.keyword,'application/vnd.google-apps.document');
...
var _HELP_MAP=(function(){var m={},d='Gmail:📧 Gmail,カレンダー:📅 カレンダー,ドキュメント:📄 ドキュメント,スプレッドシート:📊 スプシ,ドライブ:📁 ドライブ,写真保存:📸 写真保存,メモ:📝 メモ,タスク:✅ タスク,レポート:📊 レポート,リマインダー:⏰ リマインダー,誕生日リマインダー:🎂 誕生日,朝のスケジュール確認:☀️ 朝確認,URL要約:🌐 URL要約,経路・ホテル:🗺 経路/🏨 ホテル,翻訳:🌍 翻訳,文章校正:✍️ 文章校正,AIチャット:💬 AIチャット,Web検索:🔍 Web検索,天気:🌤 天気,返信作成:✉️ 返信作成,翻訳・文章校正:🌍/✍️,口調変更:🗣 口調変更,コスト管理:💰 コスト管理'.split(',');
for(var i=0;i<d.length;i++){var p=d[i].split(':');m[p[0]+'ヘルプ']=p[1];}m['ヘルプ']=1;return m;})();
function getCategoryHelp(message){return _HELP_MAP[message]||null;}
function helpText(){return'【LINE AI秘書 機能一覧】\n\n📧Gmail / 📅カレンダー / 📄ドキュメント\n📊スプレッドシート / 📁ドライブ / 📸写真保存\n📝メモ / ✅タスク / 📊レポート\n⏰リマインダー / 🎂誕生日 / ☀️朝のスケジュール確認\n🌐URL要約 / 🗺経路 / 🏨ホテル\n🌍翻訳 / ✏️文章校正 / 💬AIチャット\n🔍Web検索 / 🌤天気 / ✉️返信作成\n\n「ヘルプ」でカードメニューを表示';}
...
var cols=[];for(var i=0;i<_CAROUSEL_DATA.length;i++){var d=_CAROUSEL_DATA[i];cols.push({title:d[0],text:d[1],actions:[{type:'message',label:d[2],text:d[3]},{type:'message',label:d[4],text:d[5]},{type:'message',label:d[6],text:d[7]}]});}
return{type:'template',altText:'LINE AI秘書 機能一覧',template:{type:'carousel',columns:cols}};
}

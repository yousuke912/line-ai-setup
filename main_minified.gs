...
if (!config.ANTHROPIC_KEY) { return 'ANTHROPIC_API_KEY が未設定です。'; }
...
if (!finalReply) { finalReply = 'エラーが発生しました。\nAPIクレジット残高: https://console.anthropic.com/settings/billing'; }
var cleanHistory = [];
...
{
name: 'sheets_write',
description: '既存のスプレッドシートにデータを追加・編集する',
input_schema: {
type: 'object',
properties: {
keyword: { type: 'string', description: 'スプシ名のキーワード' },
sheet_name: { type: 'string', description: 'シート名（省略で最初）' },
mode: { type: 'string', description: 'append=行追加 / update=セル更新 / clear_and_write=クリアして書き直し', enum: ['append','clear_and_write'] },
rows: { type: 'array', items: { type: 'array' }, description: '追加する行データの配列（appendまたはclear_and_write時）' },
headers: { type: 'array', items: { type: 'string' }, description: 'ヘッダー行（clear_and_write時）' },
updates: { type: 'array', items: { type: 'object' }, description: 'セル更新リスト（update時）。各要素は{row:行番号,col:列番号,value:値}' }
},
...
{
name: 'sheets_write',
description: '既存スプレッドシートにデータ追加',
input_schema: {
type: 'object',
properties: {
keyword: { type: 'string', description: 'スプシ名のキーワード' },
sheet_name: { type: 'string', description: 'シート名（省略で最初）' },
mode: { type: 'string', description: 'append=行追加/clear_and_write=全クリア後書込', enum: ['append','clear_and_write'] },
rows: { type: 'array', items: { type: 'array' }, description: '追加する行データの配列' },
headers: { type: 'array', items: { type: 'string' }, description: 'ヘッダー行（clear_and_write時）' },
updates: { type: 'array', items: { type: 'object' }, description: '[{row,col,value}]の配列' }
},
required: ['keyword', 'mode']
}
},
...
docs:['ドキュメント','ドキュ','議事録'],
sheets:['スプレッドシート','スプシ','表','データ入力','入力'],
...
if (matched.length === 0) { return '「' + input.keyword + '」に該当する予定が見つかりません'; }
if (matched.length > 1) {
var list = matched.map(function(ev, i) {
return (i+1) + '. ' + fmtDate(ev.getStartTime(), 'M/d(E) HH:mm') + ' ' + ev.getTitle();
}).join('\n');
...
return '「' + input.keyword + '」に該当するメモが見つかりません';
}
...
return '「' + input.keyword + '」に該当するリマインダーが見つかりません';
}
...
return '「' + input.keyword + '」に該当する未完了タスクが見つかりません';
}
...
return '「' + input.keyword + '」に該当するタスクが見つかりません';
}
...
if (deleted.length === 0) { return '「' + input.keyword + '」に該当するファイルが見つかりません'; }
return '🗑 ゴミ箱に移動しました:\n' + deleted.join('\n');
}
...
if (moved.length === 0) { return '「' + input.keyword + '」に該当するファイルが見つかりません'; }
...
return '「' + input.keyword + '」に該当するファイル・フォルダが見つかりません';
}
...
if (count === 0) { return '「' + input.keyword + '」に該当するファイルが見つかりません'; }
return lines.join('\n');
}
...
if (!results.hasNext()) { return '「' + input.keyword + '」というドキュメントが見つかりません'; }
var file = results.next();
...
if (!results.hasNext()) { return '「' + input.keyword + '」というスプレッドシートが見つかりません'; }
var file = results.next();
var ss = SpreadsheetApp.openById(file.getId());
var sheet=input.sheet_name?ss.getSheetByName(input.sheet_name):ss.getActiveSheet();
if(!sheet)return 'シート「'+input.sheet_name+'」が見つかりません';
...
var _q='title contains "'+input.keyword+'" and mimeType="application/vnd.google-apps.spreadsheet" and trashed=false';
var _r=DriveApp.searchFiles(_q);
if(!_r.hasNext())return '「'+input.keyword+'」のスプレッドシートが見つかりません';
var ss=SpreadsheetApp.openById(_r.next().getId());
var sheet=input.sheet_name?ss.getSheetByName(input.sheet_name):ss.getActiveSheet();
if(!sheet)return 'シート「'+input.sheet_name+'」が見つかりません';
var rows=input.rows||[];
if(input.mode==='clear_and_write'){
sheet.clearContents();
if((input.headers||[]).length>0)sheet.appendRow(input.headers);
}
for(var i=0;i<rows.length;i++)sheet.appendRow(rows[i]);
return '✅ '+rows.length+'行を'+(input.mode==='clear_and_write'?'書き込み':'追加')+'しました\nURL: '+ss.getUrl();
}
...
return '🎂 '+input.name+'さん'+month+'/'+day+' '+hour+'時に毎年通知';
}
...
return '「' + query + '」の検索結果が見つかりません。Claudeの知識で回答してください。';
}
...
var now=new Date();
var d=props.getProperty('tokens_'+now.getFullYear()+'_'+(now.getMonth()+1));
...
return mn[now.getMonth()]+'の使用量\n入力:'+data.input+' 出力:'+data.output+' tok\n推定:約¥'+cost+'\nhttps://console.anthropic.com/settings/billing';
}
...
function getCategoryHelp(message){var m={'Gmailヘルプ':'📧 Gmail: メール確認/送信','カレンダーヘルプ':'📅 カレンダー: 予定の確認/追加/変更','ドキュメントヘルプ':'📄 ドキュメント: 作成/読み取り','スプレッドシートヘルプ':'📊 スプレッドシート: 作成/データ入力','ドライブヘルプ':'📁 ドライブ: フォルダ/ファイル管理','写真保存ヘルプ':'📸 写真保存: 送るとDriveに自動保存','メモヘルプ':'📝 メモ: 追加/表示/削除','タスクヘルプ':'✅ タスク: 追加/表示/完了/削除','レポートヘルプ':'📊 レポート: 週次/月次レポート作成','リマインダーヘルプ':'⏰ リマインダー: 日時/繰り返し通知','誕生日リマインダーヘルプ':'🎂 誕生日: 毎年自動通知','朝のスケジュール確認ヘルプ':'☀️ 朝のスケジュール確認','URL要約ヘルプ':'🌐 URL要約\n\nURLを送ると要約','経路・ホテルヘルプ':'🗺「〇〇から〇〇まで電車で」\n🏨「大阪でホテルを探して」','翻訳ヘルプ':'🌍 翻訳\n「〇〇を英語に」','文章校正ヘルプ':'✍️ 文章校正\n「この文章を丁寧にして」','AIチャットヘルプ':'💬 AIチャット\n「〇〇について教えて」','Web検索ヘルプ':'🔍 Web検索\n「〇〇を調べて」','天気ヘルプ':'🌤 天気\n「東京の天気は？」','返信作成ヘルプ':'✉️ 返信作成\n\n「返信開始」→ 作成\n「返信終了」→ 終了','翻訳・文章校正ヘルプ':'🌍/✍️「〇〇を英語に」','口調変更ヘルプ':'🗣 口調変更\n「口調変更」と送ると選択肢が出ます','コスト管理ヘルプ':'💰 コスト管理\n「残高確認」で今月の使用量を表示'};return m[message]||null;}
...
function getCategoryHelpMap(){var k='Gmailヘルプ,カレンダーヘルプ,ドキュメントヘルプ,スプレッドシートヘルプ,ドライブヘルプ,写真保存ヘルプ,メモヘルプ,タスクヘルプ,レポートヘルプ,リマインダーヘルプ,誕生日リマインダーヘルプ,朝のスケジュール確認ヘルプ,URL要約ヘルプ,経路・ホテルヘルプ,翻訳ヘルプ,文章校正ヘルプ,AIチャットヘルプ,Web検索ヘルプ,天気ヘルプ,返信作成ヘルプ,翻訳・文章校正ヘルプ,口調変更ヘルプ,コスト管理ヘルプ'.split(',');var m={};k.forEach(function(v){m[v]=1;});return m;}

...
try{var bProps=_P();if(bProps.getProperty('BRIEFING_ENABLED')!=='FALSE'){var bNow=new Date(),bH=parseInt(Utilities.formatDate(bNow,'Asia/Tokyo','HH'),10),bM=parseInt(Utilities.formatDate(bNow,'Asia/Tokyo','mm'),10),bTarget=parseInt(bProps.getProperty('BRIEFING_HOUR')||'7',10),bKey='briefing_sent_'+Utilities.formatDate(bNow,'Asia/Tokyo','yyyyMMdd');if(bH===bTarget&&bM<=2&&!bProps.getProperty(bKey)){bProps.setProperty(bKey,'true');morningBriefing();}}}catch(e){}
}

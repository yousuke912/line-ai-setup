...
props.setProperty('TONE_MENU_' + uid, 'true');
return '🗣 口調設定\n1.丁寧 2.フレンドリー 3.ビジネス 4.カスタム\n\n例:「カスタム:関西弁で」\n現在: '+(getTone(uid,props)||'丁寧');
}
var _toneMenu = props.getProperty('TONE_MENU_' + uid);
if (_toneMenu === 'true' && /^(1|2|3|丁寧|フレンドリー|ビジネス)$/.test(message)) {
props.deleteProperty('TONE_MENU_' + uid);
...
if (message === 'フォローアップON' || message === 'フォローアップon') {
PropertiesService.getScriptProperties().setProperty('FOLLOWUP_' + uid, 'TRUE');
return '✅ 予定終了後にフォローアップ通知をお届けします！\n（会議終了1時間後に「議事録を残しますか？」とお聞きします）\n停止:「フォローアップOFF」';
}
if (message === 'フォローアップOFF' || message === 'フォローアップoff') {
PropertiesService.getScriptProperties().setProperty('FOLLOWUP_' + uid, 'FALSE');
return '🔕 フォローアップ通知を停止しました。再開:「フォローアップON」';
}
if (message === '返信開始') {
...
'\n・前の会話を踏まえて行動してください' +
'\n・削除・変更は対象を確認してから実行してください。ただし追加（タスク追加・予定追加・メモ追加・リマインダー追加）は確認せずに即実行すること。「〇〇追加して」「〇〇のタスク」と言われたら確認なしで追加する。' +
'\n・【絶対厳守】メモ・タスク・予定・リマインダーの追加/削除/完了/一覧表示は、必ず対応するツールを実行すること。ツールを使わずに「保存しました」「削除しました」等と回答することは絶対禁止。ツールが失敗した場合は正直にエラーを伝えること。' +
'\n・「メモ」で始まるメッセージは、後続の内容をmemo_addで保存すること。' +
'\n・曖昧な指示は文脈から意図を推測して実行してください' +
'\n・過去の日付に予定追加しようとした場合「過去の日付ですが追加しますか？」と確認すること' +
'\n・深夜0時前後は「明日」の解釈に注意。現在日時を必ず確認して正しい日付を使うこと' +
'\n・「毎朝〇時に教えて」「定期的に通知して」→ briefing_settingツールを使う。LINEアプリのリマインダーを勧めない。' +
'\n・「〇時に教えて」「リマインドして」→ reminder_addツールを使う。外部サービスを勧めない。' +
'\n・「毎週月曜に○○」→ repeat=weekly, datetime=次の月曜の指定時刻。「毎月1日に○○」→ repeat=monthly, datetime=次の1日。「毎日○時に○○」→ repeat=daily。繰り返し指定は必ずrepeatパラメータを使う。' +
...
_T('reminder_add','リマインダー設定。「毎週月曜」→repeat=weekly,datetime=次の月曜。「毎月1日」→repeat=monthly,datetime=次の1日。「毎日朝9時」→repeat=daily,datetime=明日9時',{content:s('内容'),datetime:s('日時datetime'),repeat:e('繰返し',['none','daily','weekly','monthly','monthly_weekday']),nth_week:n('第N週'),weekday:n('曜日0=日〜6=土')},['content','datetime']),
...
_T('task_undone','タスクを未完了に戻す。「やっぱり未完了にして」「戻して」',{keyword:s('キーワード')},['keyword']),
_T('task_delete','タスク削除',{keyword:s('キーワード')},['keyword']),
_T('task_restore','削除済みタスクを復元。「消したの戻して」「復元して」',{keyword:s('キーワード')},['keyword']),
...
_T('drive_file_delete','ファイル削除。confirm=falseで対象一覧を返す。ユーザー確認後にconfirm=trueで実行',{keyword:s('削除キーワード'),folder:s('フォルダ名'),confirm:{type:'boolean',description:'true=実行/false=確認'}},['keyword']),
...
_T('docs_delete','ドキュメント削除。confirm=falseで対象を返す。確認後にconfirm=trueで実行',{keyword:s('削除キーワード'),confirm:{type:'boolean',description:'true=実行/false=確認'}},['keyword']),
...
_T('sheets_delete','スプシ削除。confirm=falseで対象を返す。確認後にconfirm=trueで実行',{keyword:s('削除キーワード'),confirm:{type:'boolean',description:'true=実行/false=確認'}},['keyword']),
...
_T('smart_search','メモ・タスク・カレンダーを横断検索。「○○について何か書いた？」「先週の○○」等',{keyword:s('検索キーワード'),range_days:n('検索範囲日数(デフォ14)')},['keyword']),
...
var groups = _G();
var keywords = {
gmail: ['メール','gmail','mail','受信','送信','添付','めーる','返信','件名'],
calendar: ['予定','カレンダー','スケジュール','会議','mtg','打ち合わせ','今日','明日','今週','来週','来月','空き','calendar','よてい','しゅうかい','アポ','面談','面接','シフト','繰り返し'],
docs: ['ドキュメント','ドキュ','文書','議事録','報告書','手順書','document'],
sheets: ['スプレッドシート','スプシ','表','シート','spreadsheet'],
drive: ['ドライブ','フォルダ','ファイル','移動','削除','検索','名前','drive'],
memo: ['メモ','覚え','記録','めも','memo','ノート'],
task: ['タスク','やること','todo','完了','締め切り','未完了','戻して','復元','元に戻','task','たすく','期限'],
reminder: ['リマインダー','通知','リマインド','誕生日','毎日','毎週','毎月','毎年','第','reminder','りまいんだー'],
...
company: ['部署','カンパニー','事業','秘書室','line事業','投稿案','ネタ','介護ブログ','学校コンサル','hp運用','部門','フォルダ確認','中身確認','会社','my-company','レポート','まとめ'],
...
if (name === 'task_undone') { return toolTaskUndone(input); }
if (name === 'task_delete') { return toolTaskDelete(input); }
if (name === 'task_restore') { return toolTaskRestore(input); }
...
if (name === 'drive_folder_create') { if(uid!=='U029395d561dbfe988aceae03cbf6affc'){return 'フォルダ作成は現在制限されています';} return toolDriveFolderCreate(input); }
...
if (name === 'smart_search') { return toolSmartSearch(input); }
...
lines.push('\n[SUGGESTION]以下を簡潔に提案してください（該当する場合のみ）:\n・日時/締切/会議があれば→「カレンダーに登録しますか？」「タスクに追加しますか？」\n・返信が必要そうなメールがあれば→「返信を作成しますか？（返信したいメールの番号を教えてください）」\n該当がなければ提案不要。');
return lines.join('\n');
}
...
var ev;
if (input.all_day) {
ev = cal.createAllDayEvent(input.title, s);
} else {
ev = cal.createEvent(input.title, s, e, { location: input.location || '', description: input.description || '' });
}
if (input.recurrence && ev) {
try {
var rec = input.recurrence;
var rule;
...
if (rule) {
ev.deleteEvent();
if (input.all_day) { ev = cal.createAllDayEventSeries(input.title, s, rule); }
else { ev = cal.createEventSeries(input.title, s, e, rule, { location: input.location || '', description: input.description || '' }); }
}
} catch(re) {}
}
var result = '追加完了: ' + input.title + ' / ' + fmtDate(s, 'M月d日(E) HH:mm') + (input.end ? '〜' + fmtDate(e, 'HH:mm') : '') + (input.recurrence ? ' [繰り返し:' + input.recurrence + ']' : '');
try {
if (!input.all_day) {
...
var content = (input.content || '').replace(/[\u3000]/g, ' ').trim();
if (!content) { return 'メモの内容が指定されていません。何を保存しますか？'; }
var sheet = getDataSheet('メモ');
if (sheet.getLastRow() === 0) { sheet.appendRow(['ID', '日時', 'タグ', '内容']); }
var id = new Date().getTime().toString();
var tag = input.tag || '';
sheet.appendRow([id, getJSTNow(), tag, content]);
...
return '保存完了: ' + input.content + (tag ? ' [' + tag + ']' : '') + '\n[SUGGESTION]メモの内容がアクションを含む場合（やるべきこと・期限あり等）、「タスクにも追加しますか？」と簡潔に提案。該当しなければ提案不要。';
}
...
// 予定終了後フォローアップ（オプトイン）
try {
var followupPref = PropertiesService.getScriptProperties().getProperty('FOLLOWUP_' + config.USER_ID);
if (followupPref === 'TRUE') {
var nowFU = new Date();
var oneHourAgo = new Date(nowFU.getTime() - 3600000);
var twoHoursAgo = new Date(nowFU.getTime() - 7200000);
var fuCal = CalendarApp.getDefaultCalendar();
var recentEvents = fuCal.getEvents(twoHoursAgo, oneHourAgo);
var fuCache = CacheService.getScriptCache();
for (var fi = 0; fi < recentEvents.length; fi++) {
var fuEvt = recentEvents[fi];
var fuKey = 'fu_' + fuEvt.getId();
if (fuCache.get(fuKey)) continue;
var fuTitle = fuEvt.getTitle();
if (fuTitle.indexOf('📋') === 0) continue;
fuCache.put(fuKey, '1', 86400);
pushToLine(config.USER_ID, '📝 「' + fuTitle + '」が終了しました。\n\n議事録やメモを残しますか？\n→「' + fuTitle + 'のメモ: ○○」と送るだけでOK！');
break;
}
}
} catch(e) {}
...
for (var mi = 1; mi < memoData.length; mi++) {
if (memoData[mi][2] === 'DELETED') continue;
if (String(memoData[mi][3] || '').indexOf(kw) !== -1) {
results.push('📝メモ: ' + memoData[mi][3] + ' (' + memoData[mi][1] + ')');
}
}
}
} catch(e) {}
...
if (taskData[ti][5] === '削除済み') continue;
if (String(taskData[ti][4] || '').indexOf(kw) !== -1) {
results.push('✅タスク: ' + taskData[ti][4] + ' [' + taskData[ti][5] + '] (' + taskData[ti][1] + ')');
}
}
}
} catch(e) {}
...
try {
var now = new Date();
var searchStart = new Date(now.getTime() - days * 86400000);
var searchEnd = new Date(now.getTime() + days * 86400000);
var cals = CalendarApp.getAllCalendars();
for (var ci = 0; ci < cals.length; ci++) {
var evts = cals[ci].getEvents(searchStart, searchEnd);
for (var ei = 0; ei < evts.length; ei++) {
if (evts[ei].getTitle().indexOf(kw) !== -1) {
results.push('📅予定: ' + evts[ei].getTitle() + ' (' + fmtDate(evts[ei].getStartTime(), 'M/d(E) HH:mm') + ')');
}
}
}
} catch(e) {}
...
if (remData[ri][4] === 'DELETED') continue;
if (String(remData[ri][3] || '').indexOf(kw) !== -1) {
results.push('🔔リマインダー: ' + remData[ri][3]);
}
}
}
} catch(e) {}
if (results.length === 0) return '「' + kw + '」に関する記録は見つかりませんでした';
return '🔍「' + kw + '」の検索結果（' + results.length + '件）:\n\n' + results.slice(0, 15).join('\n');
}
...
var taskName = (input.task || '').replace(/[\u3000]/g, ' ').trim();
if (!taskName) { return 'タスクの内容が指定されていません。何を追加しますか？'; }
var sheet = getDataSheet('タスク');
if (sheet.getLastRow() === 0) { sheet.appendRow(['ID', '追加日時', '期限', '優先度', 'タスク', '状態']); }
var id = new Date().getTime().toString();
sheet.appendRow([id, getJSTNow(), input.due || '', input.priority || '中', taskName, '未完了']);
var result = '追加完了: ' + taskName + ' [優先度:' + (input.priority || '中') + ']' + (input.due ? ' 期限:' + input.due : '');
if (input.due) {
try {
var dueStr = input.due;
if (dueStr.indexOf('+') === -1 && dueStr.indexOf('Z') === -1 && dueStr.indexOf('T') === -1) { dueStr += 'T09:00:00+09:00'; }
else if (dueStr.indexOf('+') === -1 && dueStr.indexOf('Z') === -1) { dueStr += '+09:00'; }
var dueDate = new Date(dueStr);
if (!isNaN(dueDate.getTime())) {
var calEnd = new Date(dueDate.getTime() + 3600000);
CalendarApp.getDefaultCalendar().createEvent('📋 ' + input.task, dueDate, calEnd);
result += '\n📅 カレンダーにも自動登録しました';
var dayBefore = new Date(dueDate.getTime() - 86400000);
if (dayBefore.getTime() > Date.now()) {
var rs = getDataSheet('リマインダー');
if (rs.getLastRow() === 0) { rs.appendRow(['ID','設定日時','リマインド日時','内容','送信済み','繰り返し']); }
rs.appendRow([Date.now() + '', getJSTNow(), dayBefore.getTime(), '明日期限: ' + input.task, 'FALSE', 'none']);
result += '\n🔔 前日リマインダーも自動設定しました';
}
}
} catch(e) {}
...
var data = sheet.getDataRange().getValues();
var i = _findTaskByKeyword(data, input.keyword, null);
if (i > 0 && data[i][5] !== '完了' && data[i][5] !== '削除済み') {
sheet.getRange(i+1, 6).setValue('完了');
var doneResult = '完了にしました: ' + data[i][4];
try {
var remSheet = getDataSheet('リマインダー');
if (remSheet.getLastRow() > 1) {
var remData = remSheet.getDataRange().getValues();
...
for (var ri = 1; ri < remData.length; ri++) {
if (remData[ri][4] === 'TRUE' || remData[ri][4] === 'DELETED') continue;
if (String(remData[ri][3] || '').indexOf(taskName) !== -1) {
remSheet.getRange(ri+1, 5).setValue('DELETED');
doneResult += '\n🔕 関連リマインダーも自動解除しました';
break;
}
}
...
function toolTaskUndone(input) {
var sheet = getDataSheet('タスク');
if (sheet.getLastRow() <= 1) { return '戻すタスクがありません'; }
var data = sheet.getDataRange().getValues();
for (var i = 1; i < data.length; i++) {
if (String(data[i][4] || '').indexOf(input.keyword) !== -1 && data[i][5] === '完了') {
sheet.getRange(i+1, 6).setValue('未完了');
return '未完了に戻しました: ' + data[i][4];
}
}
return '「' + input.keyword + '」に該当する完了済みタスクが見つかりませんでした';
}
function toolTaskRestore(input) {
var sheet = getDataSheet('タスク');
if (sheet.getLastRow() <= 1) { return '復元するタスクがありません'; }
var data = sheet.getDataRange().getValues();
for (var i = 1; i < data.length; i++) {
if (String(data[i][4] || '').indexOf(input.keyword) !== -1 && data[i][5] === '削除済み') {
sheet.getRange(i+1, 6).setValue('未完了');
return '復元しました: ' + data[i][4];
}
}
return '「' + input.keyword + '」に該当する削除済みタスクが見つかりませんでした';
}
...
var PROTECTED_IDS = ['1RLc-33sobz9UJ-fbcF7pluA6tAez1K3j'];
var results = DriveApp.searchFiles('title = "' + input.keyword + '"');
var targets = [];
while (results.hasNext() && targets.length < 5) {
var file = results.next();
if (PROTECTED_TYPES.indexOf(file.getMimeType()) !== -1) { continue; }
if (PROTECTED_IDS.indexOf(file.getId()) !== -1) { continue; }
if (input.folder) {
...
if (targets.length === 0) { return '「' + input.keyword + '」に該当するファイルが見つかりませんでした'; }
if (!input.confirm) {
var names = targets.map(function(f){ return '・' + f.getName() + ' (' + f.getMimeType().split('.').pop() + ')'; });
return '⚠️ 以下のファイルが見つかりました:\n' + names.join('\n') + '\n\n削除してよいかユーザーに確認してください。確認後にconfirm=trueで再度呼んでください。';
}
var deleted = [];
for (var i = 0; i < targets.length; i++) { targets[i].setTrashed(true); deleted.push(targets[i].getName()); }
return '🗑 ゴミ箱に移動しました:\n' + deleted.join('\n');
}
...
var results = DriveApp.searchFiles('title = "' + input.keyword + '" and mimeType = "application/vnd.google-apps.document" and trashed = false');
if (!results.hasNext()) { return '「' + input.keyword + '」というドキュメントが見つかりませんでした'; }
var file = results.next();
...
var results = DriveApp.searchFiles('title = "' + input.keyword + '" and mimeType = "application/vnd.google-apps.spreadsheet" and trashed = false');
if (!results.hasNext()) { return '「' + input.keyword + '」というスプレッドシートが見つかりませんでした'; }
var file = results.next();
var name = file.getName();
...
try {
var taskSheet2 = getDataSheet('タスク');
...
var props = PropertiesService.getScriptProperties();
var url = props.getProperty('CMS_SUPABASE_URL');
var key = props.getProperty('CMS_SUPABASE_KEY');
if (!url || !key) return;
var cutoff = new Date(Date.now() - 30 * 86400000).toISOString();
UrlFetchApp.fetch(url + '/rest/v1/ai_logs?created_at=lt.' + cutoff, {
